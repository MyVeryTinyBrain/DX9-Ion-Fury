...
bool PhysicsQuery::OverlapSphereTest(const Vec3& point, float radius, PxU32 layerMask, PhysicsQueryType queryType, Collider* ignoreCollider)
{
    PxSphereGeometry geometry;
    geometry.radius = radius;
    return OverlapGeometryTest(geometry, point, Quat::Identity(), layerMask, queryType, ignoreCollider);
}

Collider* PhysicsQuery::OverlapSphere(const Vec3& point, float radius, PxU32 layerMask, PhysicsQueryType queryType, Collider* ignoreCollider)
{
    PxSphereGeometry geometry;
    geometry.radius = radius;
    return OverlapGeometry(geometry, point, Quat::Identity(), layerMask, queryType, ignoreCollider);
}

std::vector<Collider*> PhysicsQuery::OverlapSphereAll(const Vec3& point, float radius, PxU32 layerMask, PhysicsQueryType queryType, unsigned int maxHit, Collider* ignoreCollider)
{
    PxSphereGeometry geometry;
    geometry.radius = radius;
    return OverlapGeometryAll(geometry, point, Quat::Identity(), layerMask, queryType, maxHit, ignoreCollider);
}

bool PhysicsQuery::OverlapBoxTest(const Vec3& point, const Vec3& extents, const Quat& rotation, PxU32 layerMask, PhysicsQueryType queryType, Collider* ignoreCollider)
{
...
Collider* PhysicsQuery::OverlapBox(const Vec3& point, const Vec3& extents, const Quat& rotation, PxU32 layerMask, PhysicsQueryType queryType, Collider* ignoreCollider)
{
    PxBoxGeometry geometry;
    geometry.halfExtents = ToPxVec3(extents);
    return OverlapGeometry(geometry, point, Quat::Identity(), layerMask, queryType, ignoreCollider);
...
std::vector<Collider*> PhysicsQuery::OverlapBoxAll(const Vec3& point, const Vec3& extents, const Quat& rotation, PxU32 layerMask, PhysicsQueryType queryType, unsigned int maxHit, Collider* ignoreCollider)
{
    PxBoxGeometry geometry;
    geometry.halfExtents = ToPxVec3(extents);
    return OverlapGeometryAll(geometry, point, Quat::Identity(), layerMask, queryType, maxHit, ignoreCollider);
